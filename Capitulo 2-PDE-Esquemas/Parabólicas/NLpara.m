%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [u]=NLpara(J,a,b)
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Esquema explicito 
%Ecuacion uxx+aux+bu=ut, x=[0,1], u(1,n)=f(x), u(J+1,n)=g(x) y condicion inicial
%u(x,0)=sin(pix)+sin(2pix);
%estencil =
%u_{j}^{n-1}=u_{j}^{n}+r(u_{j+1}^{n}-2u_{j}^{n}+u_{j-1}^{n}+adtu_{j+1,n}-u_{j}^{n}+bu_{j}^{n}
  ...
  for j=2:J
    u(j,n) = f(j) + r*(f(j+1)-2*f(j)+f(j-1));  
     ...
     u(j,n)=u(j,n-1)+r*(u(j+1,n-1)-2*u(j,n-1)+u(j-1,n-1))+...
         a*dt*(u(j+1,n-1)-u(j,n-1))+b*dt*u(j,n-1);
 end 
    u(1,n)=g1;
    u(J+1,n)=g2;
 end 
