%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [u]=Theta(theta,J)
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Esquema explicito 
%Ecuacion uxx=ut, x=[0,1], u(1,n)=f(x), u(J+1,n)=g(x) y condicion inicial
%u(x,0)=sin(pix)+sin(2pix);
%estencil = u_{j}^{n+1}=u(j,n-1)+r*(u(j+1,n-1)-2*u(j,n-1)+u(j-1,n-1))+...
     %r*(1-theta)*(u(j+1,n)+2*u(j,n)+u(j-1,n))
% para llamar la funcion, guardarla como: theta
% para llamarla: Theta(theta,J), J=malla espacial, theta=paso CN
%%%%%%%%%%%%%%%%%%%%%%%%%%%
x1=0; x2=1; %J=6; 
Nt = 50; dx = (x2-x1) / J; tf = 0.1;
dt = tf/Nt;
r = dt/(dx)^2;
%theta=0.8;
r= dt/(dx)^2;
