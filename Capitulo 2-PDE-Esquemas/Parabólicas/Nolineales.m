%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [u]=Nolineales(J)
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Esquema explicito 
%Ecuacion (u^2)xx=ut, x=[0,1], u(1,n)=f(x), u(J+1,n)=g(x) y condicion inicial
%u(x,0)=sin(pix)+sin(2pix);
%estencil = u_{j}^{n}=u_{j}^{n}+3*r(u_{j}^{n})^2)(u(j+1,n-1)-2*u(j,n-1)+u(j-1,n-1))
 ...
 if n==1
  for j=2:J
    u(j,n) = f(j) + r*(f(j+1)-2*f(j)+f(j-1));  
     ...
     for j=2:J
     u(j,n)=u(j,n-1)+3*r*(u(j,n-1)*u(j,n-1))*(u(j+1,n-1)-2*u(j,n-1)+u(j-1,n-1));         
 end 
    u(1,n)=g1;
    u(J+1,n)=g2;
 end 
