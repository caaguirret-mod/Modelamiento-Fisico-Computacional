...
%%%%%%%%%%%%%%%%%%%%%%%%%
function [y]=Ralstone(N)
%%%%%%%%%%%%%%%%%%%%%%%%%%
%Esquema de Ralstone
%Ecuacion y´=y-x^2+1, x=[a,b] y(a)=0.5, condicion inicial
% para llamar la funcion, guardarla como: Ralstone
% para llamarla: Ralstone(N), N=cantidad de iteraciones
%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
a=0.0; b=2.0;
f=@(x,y) y-x.^2+1;
...
k2=(2./3).*h.*feval(f,x(k)+(3./4).*h,y(k)+(3./4).*h.*k1);
y(k+1) = y(k) + (1./3).*k1+k2;
x(k+1)=x(k)+h;
